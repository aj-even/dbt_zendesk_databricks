...
with ticket_field_history as (
  ...
  select
    ...
    cast({{ spark__json_extract('ticket_field_history.value', 'minutes') }} as {{ dbt_utils.type_int() }} ) as target,
    {{ spark__json_extract('ticket_field_history.value', 'in_business_hours') }} = 'true' as in_business_hours
